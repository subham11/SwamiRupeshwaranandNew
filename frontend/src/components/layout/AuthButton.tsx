...
export default function AuthButton({ locale }: AuthButtonProps) {
  ...
  const labels = {
    ...
    enquiries: locale === 'en' ? 'Yagya Enquiries' : 'यज्ञ पूछताछ',
    transactions: locale === 'en' ? 'Transactions' : 'लेनदेन',
  ...
  return (
    <div className="relative hidden sm:block" ref={dropdownRef}>
      ...
      {isOpen && (
        <div
          className="absolute right-0 mt-2 w-56 rounded-lg shadow-lg border bg-white py-1 z-50"
        ...
        >
          ...
          {isEditor && (
            <>
              ...
              <Link
                href={`/${locale}/admin/yagya-enquiries`}
                onClick={() => setIsOpen(false)}
                className="flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-50 transition-colors"
                style={{ color: 'var(--color-text)' }}
              >
