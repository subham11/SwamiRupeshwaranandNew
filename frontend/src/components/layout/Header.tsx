...
export default async function Header({ locale }: { locale: AppLocale }) {
  ...
  const labels: Record<string, string> = {
    ...
    "cta.learnMore": dict?.cta?.learnMore ?? "Learn more"
  };

  // nav.*
  if (dict?.nav) {
    for (const [k, v] of Object.entries(dict.nav)) labels[`nav.${k}`] = String(v);
  }
  // navGroup.*
  if (dict?.navGroup) {
    for (const [k, v] of Object.entries(dict.navGroup)) labels[`navGroup.${k}`] = String(v);
  }
  // navDesc.*
  if (dict?.navDesc) {
    for (const [k, v] of Object.entries(dict.navDesc)) labels[`navDesc.${k}`] = String(v);
  }
    ...
    >
      ...
      <Container className="flex h-20 items-center justify-between">
        ...
        <div className="hidden lg:flex items-center">
          <MegaMenu locale={locale} groups={groups} labels={labels} />
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Mobile Menu Toggle */}
          <MobileMenu locale={locale} navGroups={groups} labels={labels} />
        </div>
      </Container>
    </header>
  );
