...
interface RazorpayProductCheckoutProps {
  /** Payment data returned from initiateProductCheckout API */
  paymentData: OrderCheckoutResponse;
  ...
  onSuccess: (response: { orderId: string; message: string }) => void;
...
export default function RazorpayProductCheckout({
  paymentData,
  accessToken,
  user,
  onSuccess,
    ...
    try {
      const options: any = {
        ...
        handler: async (response: any) => {
          try {
            const result = await verifyProductOrderPayment(
              {
